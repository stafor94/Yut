# Three.js roll QA validation v3
...
## .unit.log
```text
ok 101 - 중복 snapshot도 listener activity callback은 매번 호출하지만 상태 적용은 한 번만 한다
  ---
  duration_ms: 5.510549
  type: 'test'
  ...
# Subtest: 같은 방의 재렌더는 listener를 다시 만들지 않고 방 변경 때만 교체한다
ok 102 - 같은 방의 재렌더는 listener를 다시 만들지 않고 방 변경 때만 교체한다
  ---
  duration_ms: 2.315669
  type: 'test'
  ...
# Subtest: Strict Mode cleanup 뒤 runtime을 재주입하면 listener 처리가 정상 재개된다
ok 103 - Strict Mode cleanup 뒤 runtime을 재주입하면 listener 처리가 정상 재개된다
  ---
  duration_ms: 6.172599
  type: 'test'
  ...
# Subtest: 동일 snapshot은 한 번만 적용하고 새 sequence는 즉시 replay한다
ok 104 - 동일 snapshot은 한 번만 적용하고 새 sequence는 즉시 replay한다
  ---
  duration_ms: 1.143183
  type: 'test'
  ...
...
> playwright test --workers=1 --project=desktop-chromium tests/regression/bug-history-smoke.spec.js tests/regression/roll-mat-surface.spec.js --output=test-results/threejs-roll-regression-v3


Running 5 tests using 1 worker

  ✓  1 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:14:7 › BUG_HISTORY regression smoke › 게임 시작 직후 윷 던지기/대기 버튼 상태가 고착되지 않는다 (11.7s)
  ✘  2 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:42:7 › BUG_HISTORY regression smoke › 온라인 윷 던지기는 sequence replay 애니메이션을 표시하고 이동 직후 경로 preview를 숨긴다 (18.8s)
  ✘  3 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:271:7 › BUG_HISTORY regression smoke › host가 대리 제출한 AI 이동은 sequence 경로로 칸별 재생되고 내 이동은 중복 재생되지 않는다 (33.2s)
  ✓  4 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:503:7 › BUG_HISTORY regression smoke › timeout 벌칙은 오프라인 로컬 timeout에만 적용된다 (393ms)
  ✓  5 [desktop-chromium] › tests/regression/roll-mat-surface.spec.js:13:7 › roll mat surface regression › pending부터 결과 유지까지 같은 2D 매트 표면을 계속 표시한다 (24.2s)
  ...
  1) [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:42:7 › BUG_HISTORY regression smoke › 온라인 윷 던지기는 sequence replay 애니메이션을 표시하고 이동 직후 경로 preview를 숨긴다 
    ...
    Error: page.evaluate: Execution context was destroyed, most likely because of a navigation.
      ...
      48 |     });
      49 |
    > 50 |     const clickSequenceRollAtPerfect = () => page.evaluate(() => new Promise((resolve, reject) => {
         |                                                   ^
      51 |       const startedAt = performance.now();
      52 |       const sample = () => {
      53 |         const meter = document.querySelector('.roll-timing-meter');
        at clickSequenceRollAtPerfect (/__w/Yut/Yut/tests/regression/bug-history-smoke.spec.js:50:51)
        at /__w/Yut/Yut/tests/regression/bug-history-smoke.spec.js:117:13
    ...
    test-results/threejs-roll-regression-v3/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/test-failed-1.png
    ────────────────────────────────────────────────────────────────────────────────────────────────

    Error Context: test-results/threejs-roll-regression-v3/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/error-context.md

    attachment #3: trace (application/zip) ─────────────────────────────────────────────────────────
    test-results/threejs-roll-regression-v3/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/trace.zip
    Usage:

        npx playwright show-trace test-results/threejs-roll-regression-v3/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/trace.zip

    ────────────────────────────────────────────────────────────────────────────────────────────────

  2) [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:271:7 › BUG_HISTORY regression smoke › host가 대리 제출한 AI 이동은 sequence 경로로 칸별 재생되고 내 이동은 중복 재생되지 않는다 

    Error: 본인 optimistic 이동 애니메이션이 한 번 이상 관찰되어야 합니다.

    본인 optimistic 이동 애니메이션이 한 번 이상 관찰되어야 합니다.
    ...
    - Timeout 8000ms exceeded while waiting on the predicate
      ...
      431 |         timeout: 8_000,
      432 |         message: '본인 optimistic 이동 애니메이션이 한 번 이상 관찰되어야 합니다.',
    > 433 |       }).toBe(true);
          |          ^
      434 |       await expect.poll(async () => (await getMovingPieces()).length, { timeout: 12_000, message: '본인 optimistic 이동 애니메이션이 종료되어야 합니다.' }).toBe(0);
      435 |       await page.waitForTimeout(1_200);
      436 |       expect(await getMovingPieces(), '서버 sequence 확정 후 본인 말 이동이 다시 재생되면 안 됩니다.').toEqual([]);
        at /__w/Yut/Yut/tests/regression/bug-history-smoke.spec.js:433:10
        at actionWithLogging (/__w/Yut/Yut/tests/helpers/ui.js:19:20)
        at runQaStep (/__w/Yut/Yut/tests/helpers/ui.js:14:28)
        at /__w/Yut/Yut/tests/regression/bug-history-smoke.spec.js:403:5

    attachment #1: screenshot (image/png) ──────────────────────────────────────────────────────────
    test-results/threejs-roll-regression-v3/regression-bug-history-smo-2a1df-로-칸별-재생되고-내-이동은-중복-재생되지-않는다-desktop-chromium/test-failed-1.png
    ────────────────────────────────────────────────────────────────────────────────────────────────

    Error Context: test-results/threejs-roll-regression-v3/regression-bug-history-smo-2a1df-로-칸별-재생되고-내-이동은-중복-재생되지-않는다-desktop-chromium/error-context.md

    attachment #3: trace (application/zip) ─────────────────────────────────────────────────────────
    test-results/threejs-roll-regression-v3/regression-bug-history-smo-2a1df-로-칸별-재생되고-내-이동은-중복-재생되지-않는다-desktop-chromium/trace.zip
    Usage:

        npx playwright show-trace test-results/threejs-roll-regression-v3/regression-bug-history-smo-2a1df-로-칸별-재생되고-내-이동은-중복-재생되지-않는다-desktop-chromium/trace.zip
  ...
  2 failed
    ...
    [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:271:7 › BUG_HISTORY regression smoke › host가 대리 제출한 AI 이동은 sequence 경로로 칸별 재생되고 내 이동은 중복 재생되지 않는다 
  3 passed (1.6m)
```
...
(node:767) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///__w/Yut/Yut/tests/helpers/cleanup-qa-rooms.js is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /__w/Yut/Yut/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
QA cleanup mode=current-run, namespace=gh-29213282038-1-threejs-roll-regression-v3, workflowPrefix=없음, rooms=0
