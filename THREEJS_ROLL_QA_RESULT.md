...
> playwright test --workers=1 --project=desktop-chromium tests/regression/bug-history-smoke.spec.js tests/regression/roll-mat-surface.spec.js --output=test-results/threejs-roll-regression


Running 5 tests using 1 worker

  ✓  1 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:14:7 › BUG_HISTORY regression smoke › 게임 시작 직후 윷 던지기/대기 버튼 상태가 고착되지 않는다 (12.0s)
  ✘  2 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:42:7 › BUG_HISTORY regression smoke › 온라인 윷 던지기는 sequence replay 애니메이션을 표시하고 이동 직후 경로 preview를 숨긴다 (36.7s)
  ✓  3 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:243:7 › BUG_HISTORY regression smoke › host가 대리 제출한 AI 이동은 sequence 경로로 칸별 재생되고 내 이동은 중복 재생되지 않는다 (42.7s)
  ✓  4 [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:468:7 › BUG_HISTORY regression smoke › timeout 벌칙은 오프라인 로컬 timeout에만 적용된다 (1.4s)
  ✓  5 [desktop-chromium] › tests/regression/roll-mat-surface.spec.js:13:7 › roll mat surface regression › pending부터 결과 유지까지 같은 2D 매트 표면을 계속 표시한다 (19.5s)
  ...
  1) [desktop-chromium] › tests/regression/bug-history-smoke.spec.js:42:7 › BUG_HISTORY regression smoke › 온라인 윷 던지기는 sequence replay 애니메이션을 표시하고 이동 직후 경로 preview를 숨긴다 
    ...
    Error: primary 장면은 즉시 사라지지 않고 실제 상승·회전 프레임을 렌더링해야 합니다.

    expect(locator).toHaveAttribute(expected) failed

    Locator: locator('.roll-stage.pending-roll [data-testid="yut-roll-scene"]')
    Expected: "primary"
    Timeout: 10000ms
    Error: element(s) not found

    Call log:
      - primary 장면은 즉시 사라지지 않고 실제 상승·회전 프레임을 렌더링해야 합니다. with timeout 10000ms
      - waiting for locator('.roll-stage.pending-roll [data-testid="yut-roll-scene"]')


      121 |       const pendingFrameAtStart = await pendingScene.screenshot();
      122 |       await page.waitForTimeout(180);
    > 123 |       await expect(pendingScene, 'primary 장면은 즉시 사라지지 않고 실제 상승·회전 프레임을 렌더링해야 합니다.').toHaveAttribute('data-phase', 'primary');
          |                                                                                     ^
      124 |       const pendingFrameInMotion = await pendingScene.screenshot();
      125 |       expect(pendingFrameInMotion.equals(pendingFrameAtStart), 'primary 단계의 윷 장면은 시간에 따라 실제 프레임이 변해야 합니다.').toBe(false);
      126 |       const landingStage = page.locator('.roll-stage.resolved-from-pending.landing-roll');
        at /__w/Yut/Yut/tests/regression/bug-history-smoke.spec.js:123:85
    ...
    test-results/threejs-roll-regression/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/test-failed-1.png
    ────────────────────────────────────────────────────────────────────────────────────────────────

    Error Context: test-results/threejs-roll-regression/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/error-context.md

    attachment #3: trace (application/zip) ─────────────────────────────────────────────────────────
    test-results/threejs-roll-regression/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/trace.zip
    Usage:

        npx playwright show-trace test-results/threejs-roll-regression/regression-bug-history-smo-61972--표시하고-이동-직후-경로-preview를-숨긴다-desktop-chromium/trace.zip
...
(node:696) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///__w/Yut/Yut/tests/helpers/cleanup-qa-rooms.js is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /__w/Yut/Yut/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
QA cleanup mode=current-run, namespace=gh-29212963485-1-threejs-roll-regression, workflowPrefix=없음, rooms=0
