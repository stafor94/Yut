...
test('명시 payload를 우선하고 action id에서는 실제 epoch만 시작 시각으로 사용한다', () => {
  ...
  assert.equal(getTurnActionStartedAt({ clientActionId: 'roll_yut:p1:10000:abc123' }), 0);
  assert.equal(getTurnActionStartedAt({ clientActionId: 'roll_yut:p1:1750000000000:abc123' }), 1_750_000_000_000);
...
test('시작 시각이 없는 레거시·내부 액션은 기존 reducer 검증에 맡긴다', () => {
  assert.equal(isManualTurnActionDeadlineExpired({
    deadlineAt: 10_000,
    deadlineKind: 'move',
    expectedKind: 'move',
  ...
  assert.equal(isManualTurnActionDeadlineExpired({
    ...
    deadlineKind: 'roll',
    expectedKind: 'roll',
    clientActionId: 'roll_yut:p1:1:fixture',
    now: 20_000,
    networkGraceMs: 1_000,
  }), false);
});
