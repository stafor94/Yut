...
test('기존 시간초과 횟수가 있으면 서버가 다음 authoritative deadline을 단축한다', () => withMockNow(100000, () => {
  const state = baseState(100000, 1);
  ...
  assert.equal(result.patch.turnDeadlineAt, 100000 + 2600 + 10000);
}));
