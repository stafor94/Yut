...
test('누적 던지기 모드는 윷/모 결과를 이동 스택에 쌓고 비보너스에서 선택 상태로 전환한다', () => {
  const first = reduceAuthoritativeGameAction(
    { ...baseState(), logs: [] },
    { type: 'roll_yut', actorId: 'seat-1', payload: { allowForcedResult: true, forcedResult: { name: '윷', steps: 4, bonus: true } } },
  ...
  const second = reduceAuthoritativeGameAction(
    ...
    { type: 'roll_yut', actorId: 'seat-1', payload: { allowForcedResult: true, forcedResult: { name: '도', steps: 1 } } },
    { playMode: 'individual', pieceCount: 4, stackedRollMode: true },
  );
...
test('누적 던지기 모드는 시간초과 던지기로 이동 대기 상태가 되면 같은 턴 수동 던지기를 거부한다', () => {
  const timeoutRoll = reduceAuthoritativeGameAction(
    { ...baseState(), logs: [] },
    {
      ...
      actorId: 'seat-1',
      payload: {
        allowForcedResult: true, forcedResult: { name: '도', steps: 1 },
        timedOut: true,
  ...
  const manualRoll = reduceAuthoritativeGameAction(
    ...
    { type: 'roll_yut', actorId: 'seat-1', payload: { allowForcedResult: true, forcedResult: { name: '개', steps: 2 } } },
    { playMode: 'individual', pieceCount: 4, stackedRollMode: true },
  );
...
test('온라인 빽도에서 판 위 말이 없으면 after_roll 아이템 선택 대기를 열지 않는다', () => {
  const result = reduceAuthoritativeGameAction(
    {
      ...baseState(),
      pieces: [
    ...
    { type: 'roll_yut', actorId: 'seat-1', payload: { allowForcedResult: true, forcedResult: { name: '빽도', steps: -1 } } },
    { playMode: 'individual', pieceCount: 4, stackedRollMode: false },
  );
