...
test('온라인 누적 다시 던지기는 선택 스택을 비우고 실제 재던지기 단계로 전환한다', () => {
  const result = reduceAuthoritativeGameAction(
    {
      ...baseState(),
      roll: { name: '도', steps: 1 },
...
test('온라인 일반 다시 던지기는 실제 roll_yut 입력 기회를 열고 사용 기록을 남긴다', () => {
  const useResult = reduceAuthoritativeGameAction(
    {
      ...baseState(),
      roll: { name: '개', steps: 2 },
...
test('온라인 아이템 선택은 optimistic 처리하고 처리 중 문구 없이 방패 상태를 표시한다', () => {
