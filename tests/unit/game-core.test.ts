...
test('온라인 AI 자동 진행 effect는 기존 roll이 있어도 이동 단계를 허용한다', () => {
  const source = appSource();
  assert.equal(source.includes('if (!itemPromptTiming && roll) return undefined;'), false);
  assert.match(source, /if \(activeRoomId && canCoordinateOnlineGame && \(currentRollRef\.current \?\? roll\)\) nextRoll = currentRollRef\.current \?\? roll;/);
});

test('온라인 AI after_roll 아이템 처리 이후 같은 호출에서 추가 roll/move를 이어가지 않는다', () => {
  const source = appSource();
  assert.match(source, /if \(activeRoomId && canCoordinateOnlineGame && itemPromptTiming\) \{[\s\S]*?if \(itemPromptTiming === 'after_roll'\) \{[\s\S]*?await submitAiUseItem\(seat, \{ skipAfterRollItem: true \}\);[\s\S]*?return;[\s\S]*?\}/);
  assert.equal(source.includes('nextRoll = await submitAiRoll(seat, aiTimingZone, nextRoll)'), false);
  assert.equal(source.includes('submitAiStackedRoll(seat, null, aiTimingZone, activeRoomId ? null : selectedGoldenRoll)'), true);
});

test('온라인 AI reroll은 로컬 useItem 경로를 직접 호출하지 않는다', () => {
  const source = appSource();
  assert.equal(source.includes("await useItem('reroll', seat.id);"), true);
  assert.match(source, /if \(!activeRoomId && \(ownedItems\[seat\.id\] \?\? \[\]\)\.includes\('reroll'\)[\s\S]*?await useItem\('reroll', seat\.id\);/);
});

test('온라인 AI 빽도 이동 불가와 sequence 조회 실패는 서버 authoritative 경로로 처리한다', () => {
  const source = appSource();
  assert.match(source, /if \(activeRoomId && canCoordinateOnlineGame && nextRoll\.steps < 0\) await submitAiMovePiece\(seat, '', nextRoll, 'outer'\);/);
  assert.match(source, /if \(!latestState\) \{[\s\S]*?const syncedState = await getLatestGameState\(activeRoomId\);[\s\S]*?if \(!latestState\) return null;[\s\S]*?\}/);
});
