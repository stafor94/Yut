...
test('활성 인간 heartbeat는 방의 마지막 인간 접속 시각을 서버 Timestamp로 갱신한다', () => {
  const service = read('src/features/room/services/roomService.ts');
  const heartbeatFunction = service.slice(service.indexOf('export async function heartbeatRoomPlayer'), service.indexOf('export async function updateRoomPlayer'));
  assert.match(heartbeatFunction, /lastHumanSeenAt:\s*serverTimestamp\(\)/);
  assert.match(heartbeatFunction, /emptySince:\s*null/);
  assert.match(heartbeatFunction, /batch\.commit\(\)/);
});
