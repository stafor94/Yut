...
test('before_roll 아이템을 사용하지 않은 뒤 보유 아이템을 유지한 채 윷을 던질 수 있다', () => {
  ...
  assert.equal(rolled.patch?.itemPromptTiming, null);
...
test('before_roll 미사용 재시도는 다른 윷 입력 검증 오류를 우회하지 않는다', () => {
  const state = { ...makeState(), itemPromptTiming: null, turnDeadlineKind: 'roll' as const };
  const rolled = reduceAuthoritativeGameAction(
    state,
    {
      type: 'roll_yut',
      ...
      payload: {
        rollTimingZone: 'good',
        clientRollResult: { name: '잘못된 결과', steps: 99 },
        clientFallOccurred: false,
        clientFallCount: 0,
      },
    },
  ...
  assert.equal(rolled.status, 'rejected');
  assert.equal(rolled.reason, '클라이언트 윷 결과가 유효하지 않습니다.');
});
