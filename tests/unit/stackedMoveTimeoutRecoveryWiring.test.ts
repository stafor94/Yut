...
test('stacked timeout QA fixture는 공통 helper로 state와 sequence를 atomic commit해 action-ready 상태로 전달한다', () => {
  assert.match(qaHelperSource, /import \{ commitAuthoritativeStatePatchForQa \} from '\.\/authoritative-state-fixture\.js';/);
  assert.match(qaHelperSource, /commitAuthoritativeStatePatchForQa\(/);
  assert.match(qaHelperSource, /fixtureName: 'stacked-roll-timeout'/);
