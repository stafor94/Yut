...
test('deadline 이후 시작 시각이 기록된 일반 말 이동을 거부한다', () => {
  ...
  const result = withMockNow(deadlineAt + 100, () => reduceAuthoritativeGameAction(
    ...
    { type: 'move_piece', actorId: 'seat-1', payload: { pieceId: 'p1', branchChoice: 'outer', clientActionStartedAt: deadlineAt + 1, clientActionId: 'move-after-deadline' } } as never,
    room,
    sides,
  ));
...
test('시작 시각이 없는 레거시 말 이동은 기존 reducer 검증에 맡긴다', () => {
  const deadlineAt = 10_000;
