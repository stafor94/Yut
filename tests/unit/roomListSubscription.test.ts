...
test('활성 방마다 players listener를 한 번만 만들고 사람 인원만 집계한다', () => {
  const harness = createHarness();
  harness.controller.start();
...
test('현재 사용자의 AI 대체 자리만 남은 진행 방은 복귀를 위해 목록에 유지한다', () => {
  ...
  assert.deepEqual(getLatestPublishedRooms(harness.published), [{ id: 'room-a', title: 'A', status: 'playing', maxPlayers: 4, currentPlayers: 1, playerIds: ['viewer'] }]);
});
