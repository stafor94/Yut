...
test('같은 결과별로 독립된 재대결 bracket을 만들고 확정 순위 구간을 보존한다', () => {
  ...
  intro = aggregateTurnOrderRound(intro, Number(intro.currentRound.startAt) + 700);
...
test('재대결에서 다시 동률이면 해당 참가자만 다음 라운드로 반복한다', () => {
  ...
  intro = aggregateTurnOrderRound(intro, 2_100);
  intro = activateNextTurnOrderRound(intro, Number(intro.nextRound?.startAt));
  intro = submitRound(intro, { p1: '모', p2: '모' }, Number(intro.currentRound.startAt) + 200);
  intro = aggregateTurnOrderRound(intro, Number(intro.currentRound.startAt) + 300);
