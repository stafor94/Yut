...
test('전원이 일찍 제출해도 8초 제한시간 전에는 결과를 집계하지 않는다', () => {
  let intro = createIntro('individual', 10_000);
  intro = submitRound(intro, { p1: '모', p2: '걸', p3: '개', p4: '도' }, 11_000);
  const beforeDeadline = aggregateTurnOrderRound(intro, intro.currentRound.deadlineAt - 1);
  assert.equal(beforeDeadline.currentRound.status, 'collecting');
  assert.equal(beforeDeadline.currentRound.revealAt, undefined);

  const afterDeadline = aggregateTurnOrderRound(intro, intro.currentRound.deadlineAt);
  assert.equal(afterDeadline.currentRound.status, 'reveal-pending');
  assert.equal(afterDeadline.currentRound.revealAt, intro.currentRound.deadlineAt + TURN_ORDER_REVEAL_DELAY_MS);
});
