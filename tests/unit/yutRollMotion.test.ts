...
test('landing uses a stronger bounce and lateral roll before settling exactly', () => {
  ...
  const bouncing = getLandingMotion(LOCAL_LANDING_IMPACT_PROGRESS + 0.1, 1);
  const settled = getLandingMotion(1, 1);

  assert.equal(impact.bounceHeight, 0);
  assert.ok(bouncing.bounceHeight > 0.15);
  assert.ok(Math.abs(bouncing.rollOffsetX) >= 0.22);
  assert.ok(Math.abs(bouncing.rollOffsetZ) >= 0.09);
...
test('three-dimensional spin counts are approximately half of the previous motion', () => {
  assert.ok(PRIMARY_SPIN_TURNS_BASE <= 2.1);
  assert.ok(EXTRA_SPIN_RADIANS_PER_SECOND_BASE <= 3.2);
  assert.ok(LANDING_FLIGHT_SPIN_TURNS_BASE < 1);
});
