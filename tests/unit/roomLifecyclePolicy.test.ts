...
test('대기실은 생성 후 1시간부터 활성 인간이 있어도 삭제 대상으로 판정한다', () => {
  const createdAt = 1_000_000;
  const room = { status: 'waiting' as const, createdAt, currentPlayers: 1 };
  const activePlayers = [{ id: 'human-a', lastSeen: createdAt + ROOM_MAX_LIFETIME_MS }];
  assert.equal(getRoomLifetimeStartedAtMillis(room), createdAt);
  assert.equal(getRoomLifetimeDeadlineMillis(room), createdAt + ROOM_MAX_LIFETIME_MS);
  assert.equal(isRoomLifetimeExpired(room, createdAt + ROOM_MAX_LIFETIME_MS - 1), false);
  assert.equal(shouldDeleteRoomSnapshot(room, activePlayers, createdAt + ROOM_MAX_LIFETIME_MS - 1), false);
  assert.equal(isRoomLifetimeExpired(room, createdAt + ROOM_MAX_LIFETIME_MS), true);
  assert.equal(shouldDeleteRoomSnapshot(room, activePlayers, createdAt + ROOM_MAX_LIFETIME_MS), true);
});

test('인게임 방은 생성 시각이 아니라 게임 시작 시각부터 1시간을 계산한다', () => {
  const createdAt = 1_000_000;
  const gameStartedAt = createdAt + ROOM_MAX_LIFETIME_MS * 2;
  const room = {
    status: 'playing' as const,
    startStatus: 'playing' as const,
    createdAt,
    startCountdownEndsAt: gameStartedAt,
    currentPlayers: 1,
  };
  assert.equal(getRoomLifetimeStartedAtMillis(room), gameStartedAt);
  assert.equal(isRoomLifetimeExpired(room, gameStartedAt + ROOM_MAX_LIFETIME_MS - 1), false);
  assert.equal(isRoomLifetimeExpired(room, gameStartedAt + ROOM_MAX_LIFETIME_MS), true);
  assert.equal(shouldDeleteRoomSnapshot(room, [{ id: 'human-a', lastSeen: gameStartedAt + ROOM_MAX_LIFETIME_MS }], gameStartedAt + ROOM_MAX_LIFETIME_MS), true);
});
...
test('정상 종료된 방도 활성 인간이 남아 있고 1시간 전이면 목록과 수명주기에서 유지한다', () => {
  const now = 100_000;
...
test('기존 2시간 idle 기준은 활동 시간 보조 판정으로 유지한다', () => {
  assert.equal(ROOM_MAX_IDLE_MS, 2 * 60 * 60 * 1000);
  assert.equal(ROOM_MAX_LIFETIME_MS, 60 * 60 * 1000);
});
