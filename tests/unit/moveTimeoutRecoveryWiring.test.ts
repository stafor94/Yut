...
test('일반 roll과 누적 스택은 같은 immutable timeout context와 stable action key를 사용한다', () => {
  ...
  assert.match(hookSource, /const hasStackedMoveState = rollStack\.length > 0[\s\S]*\|\| \(roll === null && rollStackClosed\);/);
...
test('commit과 duplicate만 terminal이며 실패는 in-flight를 해제하고 제한 횟수로 재시도한다', () => {
  ...
  assert.match(hookSource, /const attemptScopeKey = `\$\{actionKey\}:\$\{onlineGameCoordinatorSeatId\}:\$\{coordinatorEpoch\}`/);
  assert.match(hookSource, /stoppedAttemptKeysRef\.current\.add\(attemptScopeKey\)/);
  assert.match(hookSource, /stoppedAttemptKeysRef\.current\.has\(attemptScopeKey\)/);
