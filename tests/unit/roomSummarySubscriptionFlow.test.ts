...
test('finished 방은 최종적으로 lobby 초기화 상태로 정리한다', () => {
  const { params, state } = setup({ room: room({ status: 'finished', title: '끝난 방', hostId: 'host-2' }) });
  applyRoomSummarySnapshot(params);
  assert.equal(params.hostingRoomUserIdRef.current, '');
  assert.equal(params.activeRoomHostIdRef.current, '');
  assert.equal(state.screen, 'lobby');
  assert.equal(state.activeRoomId, '');
  assert.equal(state.title, '');
  assert.equal(state.hostId, '');
  ...
  assert.equal(state.itemPrompt, null);
  assert.equal(state.turnIntro, null);
  assert.equal(state.endDialog, false);
  assert.equal(state.message, '게임이 종료되어 첫 대기화면으로 돌아왔습니다.');
});
