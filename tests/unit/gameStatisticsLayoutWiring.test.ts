...
test('팝업 전체와 하단 통계는 고정되고 기록 영역만 세로 스크롤된다', () => {
  ...
  assert.match(dialogStyleSource, /\.game-statistics-record-row\s*\{[\s\S]*?grid-template-columns:\s*repeat\(3,/u);
...
test('기록 카드는 Sequence 걸침 배지·타이밍 첫 글자·윷 결과로 렌더링한다', () => {
  ...
  assert.match(componentSource, /gridColumnStart:\s*group\.leadingEmptyColumns \+ 1/u);
});
