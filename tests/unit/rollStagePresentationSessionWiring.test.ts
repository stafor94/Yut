...
test('authoritative resolved timing은 queue 재구성에서 보존되고 late settle만 기존 remote hold로 복구한다', () => {
  ...
  assert.match(source, /getRemoteRollResultHoldDelayMs\(\s*authoritativeResultRevealAt,\s*ONLINE_ROLL_RESULT_HOLD_MS,\s*\)/);
