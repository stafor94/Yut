...
test('watchdog 완료는 추가 result hold 없이 RollStage queue와 presentation lock을 종료한다', () => {
  ...
  assert.match(appSource, /completion\.waitForVisualSettle\(\)/);
  assert.match(appSource, /completion\.waitForResultHold\(\)/);
});
...
test('AI scheduler는 작업 키가 해제된 최신 snapshot에서 다음 AI 좌석을 다시 예약한다', () => {
  const schedulerStart = appSource.indexOf("const actionKey = `${activeSeat.id}:${turnIndex}");
  ...
  const dependencies = appSource.indexOf('}, [activeRoomId, activeSeat,', run);
  ...
  assert.ok(run > claim);
  assert.ok(dependencies > run);
  assert.match(appSource.slice(dependencies, dependencies + 700), /turnIndex/);
  assert.match(appSource.slice(dependencies, dependencies + 700), /rollStack/);
});
