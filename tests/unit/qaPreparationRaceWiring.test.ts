...
test('WebKit 타이밍 QA는 표시 방과 Firestore 방이 일치할 때 브라우저 Auth 토큰과 cleanup 권한을 준비한다', () => {
  ...
  assert.match(timingQaSource, /const IS_WEBKIT_TIMING_QA = IS_SAFARI_TIMING \|\| process\.env\.QA_ROLE === 'safari-visible-mismatch';/);
  assert.match(timingGameSource, /if \(IS_WEBKIT_TIMING_QA\) \{\s*await page\.bringToFront\(\);\s*await expect\.poll\([\s\S]*document\.visibilityState[\s\S]*\)\.toBe\('visible'\);\s*\}/);
