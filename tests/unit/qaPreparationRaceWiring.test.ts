...
test('WebKit 타이밍 QA는 방 cleanup 권한용 Firebase Auth 토큰을 실제 준비 조건으로 polling한다', () => {
  ...
  const timingGameSource = timingQaSource.slice(timingGameStart, timingQaSource.indexOf('async function dispatchLiveTimingGesture'));

  assert.ok(timingGameStart >= 0);
  assert.match(timingGameSource, /expect\.poll\(async \(\) => \{[\s\S]*rememberRoomIdFromPage\(page\)\.catch\(\(\) => undefined\)[\s\S]*timeout: 5_000/);
  assert.match(timingGameSource, /Firebase Auth 토큰과 QA cleanup 권한이 준비되어야 합니다/);
  assert.ok(timingGameSource.indexOf('rememberRoomIdFromPage(page).catch') < timingGameSource.indexOf('addAiAndWaitUntilGameCanStart(page)'));
});
