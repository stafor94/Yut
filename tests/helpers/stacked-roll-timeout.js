...
export async function waitForStackedRollTimeoutRecovery({ actionKey, actorId, roomId, timeoutDeadlineAt }) {
  ...
  if (!recoverySnapshot) throw new Error('stacked timeout recovery 스냅샷을 찾지 못했습니다.');
  expect(recoverySnapshot.matchingCount).toBe(1);
  expect(recoverySnapshot.action?.payload).toMatchObject({
