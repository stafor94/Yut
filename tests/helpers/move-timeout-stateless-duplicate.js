...
import { isAuthoritativeCommitReduction, reduceAuthoritativeGameAction } from '../../src/features/room/services/roomAuthoritativeReducer.ts';
...
const documentsBaseUrl = (projectId) => {
  ...
  return emulator
    ? `http://${emulator}/v1/projects/${encodeURIComponent(projectId)}/databases/(default)/documents`
...
async function stageProcessedAction(fixture) {
  ...
  await commitWrites(fixture.projectId, fixture.accessToken, [{
    update: {
      ...
      fields: {
        ...encodeFields({
          clientMutationId: fixture.actionKey, sequence: fixture.coordinatorCommit.sequence,
          turnVersion: fixture.coordinatorCommit.turnVersion, type: 'move_piece', actorId: fixture.actorId,
          coordinatorSeatId: fixture.coordinatorSeatId, coordinatorEpoch: fixture.coordinatorEpoch,
        }),
...
function reduceCoordinatorMove(fixture, state) {
  const action = {
    type: 'move_piece', actorId: fixture.actorId,
    payload: {
      pieceId: fixture.targetPieceId, branchChoice: 'outer', rollStackIndex: null,
      clientActionId: fixture.actionKey, coordinatorSeatId: fixture.coordinatorSeatId,
      coordinatorEpoch: fixture.coordinatorEpoch, recoveredByCoordinator: true,
      reason: 'stalled-roll-move-timeout', timeoutDeadlineAt: fixture.timeoutDeadlineAt,
    },
  };
  const reduction = reduceAuthoritativeGameAction(state, action, {
    playMode: state.playMode, pieceCount: state.pieceCount, stackedRollMode: state.stackedRollMode,
...
async function publishCoordinatorSequence(fixture) {
  ...
  const event = {
    ...
    coordinatorSeatId: fixture.coordinatorSeatId, coordinatorEpoch: fixture.coordinatorEpoch,
    payload: reduction.payload ?? {}, schemaVersion: 2, eventSchemaVersion: 2, action, patch: reduction.patch,
    logEntries: [], expectedPreviousSequence: fixture.baselineSequence,
    clientMutationId: fixture.actionKey, clientCreatedAt: committedAt,
  };
  const stateFields = {
    ...encodeFields(reduction.patch), turnVersion: encodeValue(fixture.coordinatorCommit.turnVersion),
