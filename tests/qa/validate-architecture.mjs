...
const suiteContracts = Object.freeze({
  ...
  'mobile-galaxy-move-ack': Object.freeze({ code: 'galack', label: 'Mobile Galaxy move ACK', browsers: Object.freeze(['chromium']) }),
  'mobile-galaxy-move-start': Object.freeze({ code: 'galstart', label: 'Mobile Galaxy move start', browsers: Object.freeze(['chromium']) }),
...
if (!workflowSource.includes('seq/result.txt') || !workflowSource.includes('galaxy/result.txt')
  || !workflowSource.includes('galtime/result.txt') || !workflowSource.includes('galack/result.txt')
  || !workflowSource.includes('galstart/result.txt') || !workflowSource.includes('safvis/result.txt')
...
if (!deployWorkflowSource.includes('artifact_name: github-pages-${{ github.run_id }}-${{ github.run_attempt }}')) {
  fail('Pages deploy action이 attempt별 artifact 이름을 사용하지 않습니다.');
}
if (!deployWorkflowSource.includes('timeout: 240000')) {
  fail('Pages deploy action timeout은 4분이어야 합니다.');
}
if (deployWorkflowSource.includes('node .github/scripts/deploy-pages-with-deadline.mjs')) {
  fail('Pages 배포에 직접 REST script를 사용하지 마세요.');
}
