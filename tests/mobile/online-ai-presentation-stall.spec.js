...
test.describe('Galaxy 온라인 AI presentation lock 경합', () => {
  ...
  test('timeout recovery 중 새 snapshot을 받아도 다음 AI 던지기와 화면 전체 상태가 최신 서버 값에 수렴한다', async ({ page, context }, testInfo) => {
    ...
    const recovery = await waitForOnlineAiPresentationStallRecovery(page, fixture);
