...
export type ScoredAiCandidate = { score: number };

export const AI_SCORE_PROFILES: Record<AiDifficulty, AiScoreProfile> = {
  hard: { finish: 80, capture: 90, shortcut: 55, start: 35, stack: 20, candidateRange: 0, rerollThreshold: 55 },
  easy: { finish: 70, capture: 55, shortcut: 35, start: 25, stack: 15, candidateRange: 45, rerollThreshold: 35 },
};
...
export function chooseScoredAiCandidate<T extends ScoredAiCandidate>(candidates: T[], difficulty: AiDifficulty, random = Math.random) {
  ...
  if (random() < 0.1) return candidates[Math.min(candidates.length - 1, Math.floor(random() * candidates.length))];
