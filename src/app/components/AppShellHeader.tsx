...
export function AppShellHeader({ activeRoomId, manualSequenceSyncing, nickname, playTimeText, screen, serverStatus, serverStatusTone, soundEnabled, winner, onOpenNicknameDialog, onSyncLatestSequences, onToggleSoundEnabled }: AppShellHeaderProps) {
  const [playTimePortalTarget, setPlayTimePortalTarget] = useState<HTMLElement | null>(null);

  useEffect(() => {
    if (screen !== 'game') {
      setPlayTimePortalTarget(null);
      return;
    }
    setPlayTimePortalTarget(document.getElementById('game-log-play-time-slot'));
  }, [screen]);
  ...
  return <>
    ...
    {screen === 'game' && playTimePortalTarget && createPortal(<div data-testid="play-timer" className={`play-time ${winner ? 'stopped' : ''}`} aria-label={`현재 게임 플레이 타임 ${playTimeText}`}>{playTimeText}</div>, playTimePortalTarget)}
  </>;
}
