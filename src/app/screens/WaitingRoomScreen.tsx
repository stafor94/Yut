...
export function WaitingRoomSeatList({ seats, canManageRoom, roomInGame, localSeatId, playMode, getSeatPieceColor, onKickPlayer, onAddAI, onRemoveAI, onChangeTeam }: WaitingRoomSeatListProps) {
  return <section className="ready-list compact-ready-list" aria-label="플레이어 자리">
    {seats.map((seat) => <article className={`ready-card compact-ready-card ${seat.ready && !seat.isEmpty ? 'ready' : ''} ${seat.isAI ? 'ai' : ''} ${seat.isEmpty ? 'empty' : ''} ${seat.id === localSeatId ? 'me' : ''} ${playMode === 'team' ? (seat.team === '청팀' ? 'blue-team' : 'red-team') : ''}`} key={seat.id}>
      <div className="seat-row"><div className="seat-identity"><b style={{ background: getSeatPieceColor(seat) }}>{seat.label}</b>{seat.isEmpty ? <span className="empty-seat-badge">빈 자리</span> : <><strong>{seat.name}</strong><em className="seat-role-badge">{seat.isHost ? '방장' : seat.isAI ? 'AI' : '플레이어'}</em></>}</div><span className="seat-status-actions">{canManageRoom && seat.id !== localSeatId && !seat.isEmpty && !seat.isHost && !seat.isAI && <button className="mini-button secondary kick-player-button" onClick={() => onKickPlayer(seat)}>강퇴</button>}{seat.isEmpty && canManageRoom && <button data-testid={`add-ai-${seat.label}`} className="mini-button ai-add-button" onClick={() => onAddAI(seat.id)}>AI 추가</button>}{seat.isAI && canManageRoom && !seat.isHost && <button className="mini-button secondary ai-remove-button" onClick={() => onRemoveAI(seat.id)}>AI 제거</button>}</span>{roomInGame && !seat.isEmpty ? <span className="seat-ready-label in-game">게임중</span> : seat.ready && !seat.isEmpty && !seat.isHost && <span className="seat-ready-label">준비</span>}</div>
