export type SeatModelPlayMode = 'individual' | 'team';
export type SeatModelTeam = '청팀' | '홍팀';
export type SeatModelPieceCount = 1 | 2 | 3 | 4;

export type SeatModelSeat = {
  id: string;
  label: string;
  name: string;
  color: string;
...
export type GameSeatSnapshotLike = {
  id: string;
  label: string;
  name: string;
  color: string;
...
export const TEAM_COLORS: Record<SeatModelTeam, string> = { 청팀: '#3a78c2', 홍팀: '#d94a38' };
...
export const createSeats = (
  hostName: string,
  playMode: SeatModelPlayMode,
  playerCount: 2 | 3 | 4,
): SeatModelSeat[] => {
  const defaultTeams: SeatModelTeam[] = playMode === 'team'
    ? ['청팀', '홍팀', '청팀', '홍팀']
    : ['청팀', '청팀', '청팀', '청팀'];
...
export const spectatorsFromRoomPlayers = (players: RoomPlayerLike[]): SeatModelSeat[] => players
...
export const gameSeatSnapshotsFromSeats = (sourceSeats: SeatModelSeat[]): GameSeatSnapshotLike[] => sourceSeats
...
export const seatsFromGameSeatSnapshots = (
  gameSeats: GameSeatSnapshotLike[],
  playMode: SeatModelPlayMode,
  playerCount: 2 | 3 | 4,
): SeatModelSeat[] => {
...
export const preserveLockedGameSeats = (
  currentSeats: SeatModelSeat[],
  nextSeats: SeatModelSeat[],
): SeatModelSeat[] => nextSeats.map((nextSeat) => {
...
export const makePieces = (
  seats: SeatModelSeat[],
  pieceCount: SeatModelPieceCount,
  mode: SeatModelPlayMode = 'individual',
) => {
  const activeSeats = seats.filter((seat) => !seat.isEmpty);
  if (mode === 'team') {
    return (['청팀', '홍팀'] as SeatModelTeam[]).flatMap((team) => {
