...
import type { SequenceStateSnapshot, Screen } from '../appState';
...
const hasPlayableGameState = (state: SyncedGameState) => Array.isArray(state.pieces) && state.pieces.length > 0 && (Boolean(state.waitingForPlayersReady) || Array.isArray(state.turnOrderIds));

export function useGameSyncSubscription({ activeRoomId, screen, lastAppliedSequenceRef, lastAppliedStateVersionRef, applyingSyncedStateRef, setScreen, replayMissingSequencesThenApply, applySyncedStateSnapshot, subscribe = subscribeGameState }: GameSyncSubscriptionParams) {
  useEffect(() => {
    ...
    return subscribe(activeRoomId, (state) => {
      ...
      if (screen === 'waitingRoom' && hasPlayableGameState(state)) setScreen('game');
  ...
  }, [activeRoomId, screen, lastAppliedSequenceRef, lastAppliedStateVersionRef, applyingSyncedStateRef, subscribe]);
}
