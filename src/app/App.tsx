...
const TURN_ORDER_STALE_TIMEOUT_MS = 3000;
const TURN_ORDER_REVEAL_MS = 5000;
const TURN_ORDER_NEXT_PLAYER_DELAY_MS = 3200;
const TURN_ORDER_ROLL_ANIMATION_MS = 2600;
const TURN_ORDER_ROLL_GUARD_TIMEOUT_MS = TURN_ORDER_ROLL_ANIMATION_MS + 2000;
...
export function App() {
  ...
  const turnOrderRollingRef = useRef(false);
  const turnOrderRollingGuardRef = useRef<{ seatId: string; startedAt: number } | null>(null);
  ...
  const canUseMoveButton = Boolean(roll && activeSeat && isMyTurn && !winner && !turnOrderIntro && !movingPieceId && !rollResultHolding && !trapPlacementActive && canMoveSelectedPiece);
  const canRollNow = Boolean(isMyTurn && !winner && !turnOrderIntro && !movingPieceId && !roll && !isRollLocked && !trapPlacementActive);
  const currentTurnOrderSeat = turnOrderPhase.active ? playableSeats[turnOrderPhase.index] : undefined;
  const currentTurnOrderSeatRolled = Boolean(currentTurnOrderSeat && turnOrderPhase.rolls.some((rollEntry) => rollEntry.seat.id === currentTurnOrderSeat.id));
  const canControlTurnOrderTimeout = !activeRoomId || isRoomHost;
  const isTurnOrderTimedOut = Boolean(turnOrderPhase.active && turnOrderPhase.deadline > 0 && turnOrderClock >= turnOrderPhase.deadline && !currentTurnOrderSeatRolled);
  const canForceTurnOrderProgress = Boolean(isTurnOrderTimedOut && canControlTurnOrderTimeout);
  const turnOrderTimerText = (() => {
    if (!turnOrderPhase.active) return '';
    if (turnOrderPhase.readyAt > turnOrderClock) {
      const label = currentTurnOrderSeatRolled ? '결과 확인' : '시작까지';
      return `${label} ${Math.max(0, Math.floor((turnOrderPhase.readyAt - turnOrderClock) / 1000))}초`;
    }
    if (turnOrderPhase.deadline > 0 && turnOrderClock >= turnOrderPhase.deadline && !currentTurnOrderSeatRolled) return '자동 진행 중...';
  ...
  useEffect(() => {
    turnOrderRollingRef.current = false;
    turnOrderRollingGuardRef.current = null;
  ...
  useEffect(() => {
    ...
    if (!seat) {
      if (!activeRoomId || isRoomHost) finishTurnOrderCeremony(turnOrderPhase.rolls);
      return undefined;
    }
    if (activeRoomId && !isRoomHost && (seat.id !== localSeatId || seat.isAI)) return undefined;
    const now = Date.now();
    const readyWait = Math.max(0, turnOrderPhase.readyAt - now);
    if (readyWait > 0) {
      const timer = window.setTimeout(() => setTurnOrderClock(Date.now()), readyWait);
      return () => window.clearTimeout(timer);
    }
    const remaining = Math.max(0, turnOrderPhase.deadline - now);
    const delayMs = seat.isAI ? 700 : remaining;
    const timer = window.setTimeout(() => rollForTurnOrder(), delayMs);
    return () => window.clearTimeout(timer);
  }, [activeRoomId, isRoomHost, localSeatId, playableSeats, turnOrderPhase]);
  ...
  useEffect(() => {
    ...
    const hasSeatRolled = turnOrderPhase.rolls.some((rollEntry) => rollEntry.seat.id === seat.id);
    if (!hasSeatRolled && turnOrderClock >= turnOrderPhase.deadline) {
      rollForTurnOrder(true);
      return;
    }
    if (turnOrderClock < turnOrderPhase.deadline + TURN_ORDER_STALE_TIMEOUT_MS) return;
    const rolledSeatIds = new Set(turnOrderPhase.rolls.map((rollEntry) => rollEntry.seat.id));
    ...
    if (nextUnrolledIndex >= 0) {
      const now = Date.now();
      setTurnOrderPhase({ ...turnOrderPhase, index: nextUnrolledIndex, readyAt: now, deadline: now + TURN_ORDER_TIMEOUT_MS });
      return;
    }
    finishTurnOrderCeremony(turnOrderPhase.rolls);
  }, [canControlTurnOrderTimeout, playableSeats, turnOrderClock, turnOrderPhase]);
  ...
  async function submitRemoteAction(type: GameAction['type'], payload: Record<string, unknown> = {}) {
    ...
    await submitGameAction(activeRoomId, { type, actorId: localSeatId, payload });
  }
  ...
  async function handleRemoteGameAction(action: GameAction) {
    ...
    try {
      ...
      if (!actorSeat) { shouldMarkProcessed = true; return; }
      if (!canActorAct) {
        shouldMarkProcessed = !movingPieceId;
        return;
      }
      if (action.type !== 'place_trap' && pendingTrapPlacement) return;
      ...
      if (action.type === 'place_trap') {
        if (!pendingTrapPlacement || pendingTrapPlacement.ownerId !== action.actorId) return;
  ...
  function startLocalGame() {
    ...
    setLogs([makeLog('3초 뒤 순서 정하기를 시작합니다. 각자 10초 안에 윷을 던져주세요.')]);
    setTurnOrderIntro(null);
    const turnOrderReadyAt = Date.now() + TURN_ORDER_START_DELAY_MS;
    setTurnOrderPhase({ active: true, index: 0, rolls: [], deadline: turnOrderReadyAt + TURN_ORDER_TIMEOUT_MS, readyAt: turnOrderReadyAt });
  ...
  function rollForTurnOrder(fromRemote = false) {
    ...
    const seat = playableSeats[turnOrderPhase.index];
    const guard = turnOrderRollingGuardRef.current;
    if (guard && Date.now() - guard.startedAt > TURN_ORDER_ROLL_GUARD_TIMEOUT_MS) {
      turnOrderRollingRef.current = false;
      turnOrderRollingGuardRef.current = null;
    }
    if (turnOrderRollingRef.current && (!seat || turnOrderRollingGuardRef.current?.seatId === seat.id)) return;
    const rolledSeatIds = new Set(turnOrderPhase.rolls.map((rollEntry) => rollEntry.seat.id));
    ...
    if (!seat) { finishTurnOrderCeremony(turnOrderPhase.rolls); return; }
    turnOrderRollingGuardRef.current = { seatId: seat.id, startedAt: Date.now() };
    const rolled = rollYutResult(undefined, false);
    ...
    if (turnOrderPhase.index + 1 >= playableSeats.length) {
      ...
      turnOrderRollingRef.current = false;
      turnOrderRollingGuardRef.current = null;
      setTurnOrderPhase({ active: true, index: turnOrderPhase.index + 1, rolls: nextRolls, readyAt, deadline: readyAt + TURN_ORDER_TIMEOUT_MS });
    }
  }
  ...
  function moveSelectedPiece(extraSteps = 0) {
    if (!roll || !activeSeat || !isMyTurn || turnOrderIntro || rollResultHolding || pendingTrapPlacement) return false;
  ...
  function finishGame() {
    ...
    if (finishedRoomId && wasHost) {
      void updateRoomStatus(finishedRoomId, 'finished');
    ...
    {screen === 'waitingRoom' && (() => {
      ...
      return <section data-testid="waiting-room" className={`panel waiting-room compact-waiting-room ${isRoomHost ? 'host-view' : 'player-view'}`} aria-label="방 대기 화면">
        ...
        <div className="waiting-main-grid">
          {(isRoomHost || playMode === 'team') && <section className="waiting-setup-card" aria-label="방 설정과 시작 조건">
          ...
          <section className="ready-list compact-ready-list" aria-label="플레이어 자리">
            {seats.map((seat) => <article className={`ready-card compact-ready-card ${seat.isAI ? 'ai' : ''} ${seat.isEmpty ? 'empty' : ''} ${seat.id === localSeatId ? 'me' : ''} ${playMode === 'team' ? (seat.team === '청팀' ? 'blue-team' : 'red-team') : ''}`} key={seat.id}>
              ...
              {playMode === 'team' && <div className="team-card-selector" role="group" aria-label={`${seat.label} 팀 선택`}>{(['청팀', '홍팀'] as Team[]).map((team) => <button type="button" key={team} className={`team-card-option ${team === seat.team ? 'active' : ''} ${team === '청팀' ? 'blue' : 'red'}`} disabled={!isRoomHost} onClick={() => changeTeam(seat.id, team)}>{team}</button>)}</div>}
