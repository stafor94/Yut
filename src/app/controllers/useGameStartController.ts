...
import type { GameLog, Seat, SequenceStateSnapshot, TurnOrderRoll } from '../appState';
import { START_REQUEST_TIMEOUT_MS, TURN_ORDER_AI_DELAY_SPREAD_MS, TURN_ORDER_AI_MIN_DELAY_MS, TURN_ORDER_FINAL_HOLD_MS, TURN_ORDER_PRESENCE_FALLBACK_MS, TURN_ORDER_ROLL_ANIMATION_MS, TURN_ORDER_TIMEOUT_FALLBACK_GRACE_MS } from '../config/gameTimings';
import { TURN_ACTION_TIMEOUT_MS } from '../../features/room/services/roomTiming';
import { getTurnOrderFromRolls, getTurnOrderLogTexts, getTurnOrderScore, resolveTurnOrderRolls } from '../flows/turnOrderFlow';
...
export function useGameStartController(ctx: any) {
  ...
  const { measureFirebaseLatency, delay, getQaRequestRoomGameStartDelayMs, getQaInitializeGameStateDelayMs, getStartGameBlockMessage,
    ...
    applySyncedStateSnapshot, replayMissingSequencesThenApply, getLocalActionKey, withActorLogPayload, playSfx, clearRoll,
  } = helpers;
  ...
  function completeTurnOrderRolls(rolls: TurnOrderRoll[]) { const rolledSeatIds = new Set(rolls.map((rollEntry) => rollEntry.seat.id)); const fallbackRolls = ctx.playableSeats.filter((seat: Seat) => !rolledSeatIds.has(seat.id)).map((seat: Seat) => { const rolled = rollYutResult(undefined, false); return { seat, result: rolled.result, rollOffRound: 1 }; }); return [...rolls, ...fallbackRolls]; }
  function makeTurnOrderCeremonyPatch(sourceRolls: TurnOrderRoll[], currentLogs: GameLog[]) {
    ...
    const timeoutLogTexts = ctx.playableSeats.filter((seat: Seat) => !rolledSeatIds.has(seat.id)).map((seat: Seat) => `${getSeatDisplayName(seat)}님이 시간 초과로 자동 순서 정하기 굴림 처리되었습니다.`);
    const completedRolls = completeTurnOrderRolls(sourceRolls); const { rankedRolls, turnOrder: orderedSeats } = getTurnOrderFromRolls(completedRolls, ctx.playModeForTurnOrder); const nextPieces = makePieces(orderedSeats, ctx.pieceCount, ctx.playModeForTurnOrder); const nextTurnOrderPhase = { active: false, index: 0, rolls: completedRolls, deadline: 0, readyAt: 0 }; const nextTurnOrderIds = orderedSeats.map((seat: { id: string }) => seat.id); const nextBoardItems = ctx.itemMode ? spawnInitialBoardItems(4, 8) : []; const { intro: nextTurnOrderIntro } = createTurnOrderIntro(orderedSeats, { getSeatPieceColor, playMode: ctx.playModeForTurnOrder, finalHoldMs: TURN_ORDER_FINAL_HOLD_MS }); const nextGameStartedAt = nextTurnOrderIntro.readyAt; const finalOrderLog = formatTurnOrderSummary(orderedSeats, getSeatDisplayName); const ceremonyLogs = [...timeoutLogTexts, ...getTurnOrderLogTexts(rankedRolls, orderedSeats, getSeatDisplayName)].reverse().map((text) => makeLog(text));
    return { local: { completedRolls, orderedSeats, nextPieces, nextBoardItems, nextTurnOrderIds, nextTurnOrderPhase, nextTurnOrderIntro, nextGameStartedAt, finalOrderLog, nextLogs: [...ceremonyLogs, ...currentLogs] } };
  }
  function finishTurnOrderCeremony(rolls: TurnOrderRoll[]) { if (!ctx.canCoordinateOnlineGame) return; const { local } = makeTurnOrderCeremonyPatch(rolls, ctx.logs); if (ctx.activeRoomId) pendingSequenceMetaRef.current = { type: 'turn_order_resolved', actorId: ctx.localSeatId, clientMutationId: `turn_order_resolved:${ctx.activeRoomId}:${ctx.startRequestVersion}:${local.nextTurnOrderIntro.readyAt}`, payload: { startRequestVersion: ctx.startRequestVersion, turnOrderIds: local.nextTurnOrderIds, completedRollCount: local.completedRolls.length, slotUntil: local.nextTurnOrderIntro.slotUntil, readyAt: local.nextTurnOrderIntro.readyAt } }; resetGameBoard(local.nextPieces, local.nextBoardItems); setTurnOrderIds(local.nextTurnOrderIds); setInitialTurnOrderIds(local.nextTurnOrderIds); setCompletedSeatIds([]); setRankingSeatIds([]); setGameEndMode(''); setLastFinishedSeatId(''); setAuthoritativeWinner(''); setContinuationRound(0); setTurnOrderPhase(local.nextTurnOrderPhase); setTurnOrderIntro(local.nextTurnOrderIntro); setGameStartedAt(local.nextGameStartedAt); setLogs(local.nextLogs); }
  function rollForTurnOrder(fromRemote = false, requestedSeatId = ctx.localSeatId) { if (!ctx.turnOrderPhase.active || Date.now() < ctx.turnOrderPhase.readyAt) return; const seat = ctx.playableSeats.find((candidate: Seat) => candidate.id === requestedSeatId); if (!seat) return; const rolled = rollYutResult(undefined, false); const rollEntry = { seat, result: rolled.result, rollOffRound: 1 }; const nextAnimation = { id: Date.now(), result: rolled.result, sticks: rolled.sticks, turnOrder: true }; const logText = `${getSeatDisplayName(seat)}님이 순서 정하기에서 ${rolled.result.name}(${getTurnOrderScore(rolled.result)}점)를 던졌습니다.`; const canSubmitOnlineTurnOrderRoll = !ctx.activeRoomId || fromRemote || requestedSeatId === ctx.localSeatId || Boolean(seat.isAI && ctx.canCoordinateOnlineGame); if (!canSubmitOnlineTurnOrderRoll) return; const clientMutationId = getLocalActionKey('turn_order_roll', { actorId: requestedSeatId }); if (ctx.activeRoomId && !fromRemote) pendingSequenceMetaRef.current = { type: 'turn_order_roll', actorId: requestedSeatId, clientMutationId, payload: { rollIndex: ctx.turnOrderPhase.rolls.length }, action: { type: 'turn_order_roll', actorId: requestedSeatId, payload: withActorLogPayload({ clientActionId: clientMutationId }, seat) } }; let accepted = false; setTurnOrderPhase((current: any) => { if (!current.active || current.rolls.some((entry: TurnOrderRoll) => entry.seat.id === seat.id)) return current; accepted = true; return { ...current, rolls: [...current.rolls, rollEntry] }; }); if (!accepted) return; setRollAnimation(nextAnimation); playSfx('roll' as SoundEffect); window.setTimeout(() => setRollAnimation(null), TURN_ORDER_ROLL_ANIMATION_MS); setLogs((current: GameLog[]) => [makeLog(logText), ...current]); }
  ...
  useTurnOrderAutoFinish({ playableSeats: ctx.playableSeats, turnOrderPhase: ctx.turnOrderPhase, finishTurnOrderCeremony });
  ...
  useEffect(() => { if (!ctx.turnOrderPhase.active) return undefined; const localSeat = ctx.playableSeats.find((seat: Seat) => seat.id === ctx.localSeatId); if (!localSeat || localSeat.isAI || ctx.turnOrderPhase.rolls.some((entry: TurnOrderRoll) => entry.seat.id === ctx.localSeatId)) return undefined; if (ctx.activeRoomId) return undefined; const delayMs = Math.max(0, ctx.turnOrderPhase.deadline - Date.now()); const timer = window.setTimeout(() => rollForTurnOrder(false, localSeat.id), delayMs); return () => window.clearTimeout(timer); }, [ctx.activeRoomId, ctx.localSeatId, ctx.playableSeats, ctx.turnOrderPhase]);
  useEffect(() => { if (!ctx.turnOrderPhase.active) return undefined; if (!ctx.activeRoomId || !ctx.canCoordinateOnlineGame) return undefined; const rolledIds = new Set(ctx.turnOrderPhase.rolls.map((entry: TurnOrderRoll) => entry.seat.id)); const timers = ctx.playableSeats.filter((seat: Seat) => seat.isAI && !rolledIds.has(seat.id)).map((seat: Seat) => window.setTimeout(() => rollForTurnOrder(false, seat.id), Math.max(0, ctx.turnOrderPhase.readyAt - Date.now()) + TURN_ORDER_AI_MIN_DELAY_MS + Math.random() * TURN_ORDER_AI_DELAY_SPREAD_MS)); return () => timers.forEach((timer: number) => window.clearTimeout(timer)); }, [ctx.activeRoomId, ctx.canCoordinateOnlineGame, ctx.playableSeats, ctx.turnOrderPhase]);
  useEffect(() => { if (!ctx.turnOrderPhase.active || ctx.turnOrderPhase.readyAt > ctx.turnOrderClock || ctx.turnOrderPhase.deadline <= 0) return; if (!ctx.canCoordinateOnlineGame) return; if (ctx.turnOrderClock < ctx.turnOrderPhase.deadline) return; finishTurnOrderCeremony(ctx.turnOrderPhase.rolls); }, [ctx.activeRoomId, ctx.canCoordinateOnlineGame, ctx.turnOrderClock, ctx.turnOrderPhase]);
  useEffect(() => { const due = Boolean(ctx.turnOrderPhase.active && ctx.turnOrderPhase.deadline > 0 && ctx.turnOrderClock >= ctx.turnOrderPhase.deadline + TURN_ORDER_TIMEOUT_FALLBACK_GRACE_MS); if (!due || !ctx.canCoordinateOnlineGame) return; finishTurnOrderCeremony(ctx.turnOrderPhase.rolls); }, [ctx.activeRoomId, ctx.canCoordinateOnlineGame, ctx.turnOrderClock, ctx.turnOrderPhase]);
  ...
  return { handleStartGame, cancelStartCountdown, rollForTurnOrder, countdown: ctx.countdown, startRequestPending: ctx.startRequestPending, initialGameEntryPending: ctx.initialGameEntryPending };
}
